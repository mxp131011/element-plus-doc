...
export default function findTagResult (list: Node[], result: any[], document: vscode.TextDocument, pattern: RegExp) {
    ...
    if (pattern.test(list[i].tag as string)) {
      ...
      result.push({
        range: new vscode.Range(
          ...
          document.positionAt(list[i].start + Number(list[i].tag?.length) + 1),
        ),
        target: vscode.Uri.parse(
          `http://element-plus.org/zh-CN/component/${componentName ? componentName.mapValue : list[i].tag?.replace(pattern, '')}.html`),
      });
    }
