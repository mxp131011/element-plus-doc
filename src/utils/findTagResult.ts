...
export default function findTagResult(
  list: Node[],
  result: any[],
  ...
  for (let i = 0; i < list.length; i++) {
    if (pattern.test(list[i].tag as string)) {
      const componentName = mapValue.find(
        (val) => val.value === list[i].tag?.replace(pattern, "")
      );
      result.push({
        ...
        target: vscode.Uri.parse(
          `http://element-plus.org/zh-CN/component/${
            componentName
              ? componentName.mapValue
              : list[i].tag?.replace(pattern, "")
          }.html`
        ),
      });
    }
