...
const doc: TagDoc.TagDocInstance = {
  ...
  attributes: [
    ...
    },
    {
      name: 'load',
      description: {
        cn: '加载子树数据的方法，仅当  `lazy` 属性为 `true` 时生效, 绑定方法说明： function(node, resolve)，node为当前点击的节点，resolve为数据加载完成的回调(必须调用)',
        en: 'method for loading subtree data, only works when `lazy` is `true`，binding function：function(node, resolve) ,node is the current clicked node, resolve is the callback when data loading is complete (must be called)',
      },
      type: 'function',
      default: '—',
      value: [],
    ...
    },
    {
      ...
      description: {
        cn: '对树节点进行筛选时执行的方法， 返回 `false` 则表示这个节点会被隐藏',
    ...
    },
    {
      name: 'lazy',
      description: {
        cn: '是否懒加载子节点，需与 `load` 方法结合使用',
        en: 'whether to lazy load leaf node, used with `load` attribute',
      },
      type: 'boolean',
      default: 'false',
      value: [],
    ...
    },
    {
      name: 'allow-drag',
      description: {
        cn: '判断节点能否被拖拽 如果返回 `false` ，节点不能被拖动',
        en: 'this function will be executed before dragging a node. If `false` is returned, the node can not be dragged',
      },
      type: 'function(node)',
      default: '—',
      value: [],
    },
    {
      name: 'allow-drop',
      description: {
        cn: "拖拽时判定目标节点能否成为拖动目标位置。 如果返回 `false` ，拖动节点不能被拖放到目标节点。 `type` 参数有三种情况：'prev'、'inner' 和 'next'，分别表示放置在目标节点前、插入至目标节点和放置在目标节点后",
        en: "this function will be executed before the dragging node is dropped. If `false` is returned, the dragging node can not be dropped at the target node. `type` has three possible values: 'prev' (inserting the dragging node before the target node), 'inner' (inserting the dragging node to the target node) and 'next' (inserting the dragging node after the target node)",
      },
  ...
  exposes: [
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '为节点设置新数据，只有当设置 `node-key` 属性的时候才可用',
        en: 'set new data to node, only works when `node-key` is assigned',
      },
      param: {
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '如果节点可以被选中，(`show-checkbox` 为 `true`), 本方法将返回当前选中节点的数组',
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '设置目前勾选的节点，使用此方法必须提前设置 `node-key` 属性',
        en: 'set certain nodes to be checked, only works when `node-key` is assigned',
      },
      param: {
        cn: '要选中的节点构成的数组',
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '若节点可用被选中 (`show-checkbox` 为 `true`), 它将返回当前选中节点 key 的数组',
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '设置目前选中的节点，使用此方法必须设置 `node-key` 属性',
        en: 'set certain nodes to be checked, only works when `node-key` is assigned',
      },
      param: {
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '设置节点是否被选中, 使用此方法必须设置 `node-key` 属性',
        en: 'set node to be checked or not, only works when `node-key` is assigned',
      },
      param: {
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '如果节点可用被选中 (`show-checkbox` 为 `true`), 它将返回当前半选中的节点组成的数组',
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '若节点可被选中(`show-checkbox` 为 `true`)，则返回目前半选中的节点的 key 所组成的数组',
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '通过 key 设置某个节点的当前选中状态，使用此方法必须设置 `node-key` 属性',
        en: 'set highlighted node by key, only works when `node-key` is assigned',
      },
      param: {
    ...
    },
    {
      ...
      type: 'function',
      description: {
        cn: '设置节点为选中状态，使用此方法必须设置 `node-key` 属性',
        en: 'set highlighted node, only works when `node-key` is assigned',
      },
      param: {
  ...
  slots: [
    {
      ...
      description: {
        ...
        en: 'Custom content for tree nodes. The scope parameter is `{ node, data }`',
      },
    },
  ],
